...
class TestJSONPlaceholderAPI:
    ...
    def test_fetch_user_successfully(self):       
        ...
        print(f"GET {url}")                             # Print the URL being requested (for clarity)
        print(f"Status Code: {response.status_code}")   # Print the HTTP status code (e.g., 200 for success)   
        print(f"Response: {response.json()}")           # Print the response body parsed as JSON (dictionary)
    ...
    def test_create_new_post(self, title, body, userId):
        ...
        print(f"Response: {response.json()}")           # Print the JSON response body from the API
    ...
    def test_handle_nonexistent_user(self):
        ...
        print(f"Response: {response.json()}")           # Print the JSON response 
